...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .map((e, index) => str.charCodeAt(index))
...
function isPalindrome(str) {
  ...
  const num = Math.floor(string.length / 2);
  const strStart = string.slice(0, num);
  const strEnd = string.slice(-num);
  return strStart === strEnd.split('').reverse().join('');
}
